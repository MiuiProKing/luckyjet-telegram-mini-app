(() => {
  ...
  function panelHtml() {
    return `<div class="oa-head"><div class="oa-logo">AP</div><div><div class="oa-title">${t("title")}</div><div class="oa-sub">Telegram ID: ${escapeHtml(currentTelegramId())} · ${lang().toUpperCase()}</div></div><button class="oa-close" id="ownerAdminClose" type="button">✕</button></div>
      ...
      <div class="oa-grid"><section class="oa-card"><h3>${t("create")}</h3><div class="oa-row"><select class="oa-field" id="ownerPlan"><option value="1">1 ДЕНЬ</option><option value="7">7 ДНЕЙ</option><option value="30" selected>30 ДНЕЙ</option><option value="90">90 ДНЕЙ</option><option value="custom">СВОЙ СРОК</option><option value="lifetime">БЕЗ СРОКА</option></select><input class="oa-field" id="ownerDays" type="number" inputmode="numeric" min="1" max="3650" value="30" placeholder="Количество дней"><select class="oa-field" id="ownerDevices"><option value="1">1 DEVICE</option><option value="2">2 DEVICES</option></select><input class="oa-field" id="ownerTelegramId" type="number" inputmode="numeric" placeholder="Telegram ID (необязательно)"><input class="oa-field" id="ownerNote" placeholder="${t("comment")}"><button class="oa-btn gold" id="ownerCreateKey" type="button">СОЗДАТЬ КОД</button></div><div class="oa-status" id="ownerLicenseStatus"></div><div class="oa-filters"><button class="oa-filter active" data-license-filter="all">${t("all")}</button><button class="oa-filter" data-license-filter="active">${t("active")}</button><button class="oa-filter" data-license-filter="revoked">${t("revoked")}</button></div><div class="oa-list" id="ownerLicenses"></div></section>
  ...
  function bindItems(){
    ...
    document.querySelectorAll("[data-owner-extend]").forEach(b=>b.onclick=async()=>{const raw=prompt("На сколько дней продлить?","30");if(raw===null)return;const days=Math.min(3650,Math.max(1,Number(raw)||30));await api("extend_license",{licenseId:b.dataset.ownerExtend,days});toast(`Продлено на ${days} дней`);await loadAdmin();});
    document.querySelectorAll("[data-owner-license]").forEach(b=>b.onclick=()=>{const field=document.getElementById("ownerTelegramId");if(field)field.value=b.dataset.ownerLicense||"";const status=document.getElementById("ownerLicenseStatus");if(status)status.textContent=`Выбран пользователь ID ${b.dataset.ownerLicense}. Укажите срок и нажмите «Создать код».`;document.getElementById("ownerCreateKey")?.scrollIntoView({behavior:"smooth",block:"center"});});
  ...
  function humanError(error){const code=String(error?.message||error||"UNKNOWN_ERROR");const map={TELEGRAM_AUTH_MISSING:"Нет данных Telegram. Перезапустите Mini App из бота.",TELEGRAM_AUTH_INVALID:"Telegram не подтвердил вход. Перезапустите Mini App.",TELEGRAM_AUTH_EXPIRED:"Сессия Telegram устарела. Перезапустите Mini App.",ADMIN_ACCESS_DENIED:"Этот Telegram ID не является администратором.",NETWORK_ERROR:"Не удалось подключиться к серверу Supabase.",SUPABASE_NOT_CONFIGURED:"Сервер Supabase не настроен.",INVALID_PLAN:"Неверный срок лицензии.",USER_NOT_FOUND:"Пользователь с таким Telegram ID ещё не запускал приложение."};return map[code]||`${t("error")}: ${code}`;}
  ...
  function buildOverlay(){
    ...
    const plan=document.getElementById("ownerPlan"),days=document.getElementById("ownerDays");
    const syncDays=()=>{const lifetime=plan.value==="lifetime";days.disabled=lifetime;if(!lifetime&&Number(plan.value)>0)days.value=plan.value;};
    plan.onchange=syncDays;syncDays();
    document.getElementById("ownerCreateKey").onclick=async()=>{const status=document.getElementById("ownerLicenseStatus");status.textContent="…";try{const lifetime=plan.value==="lifetime";const durationDays=lifetime?null:Math.min(3650,Math.max(1,Number(days.value)||30));const planId=lifetime?"lifetime":durationDays===1?"pro_1":durationDays===7?"pro_7":durationDays===90?"pro_90":"pro_30";const boundTelegramId=Number(document.getElementById("ownerTelegramId").value)||null;const result=await api("create_license",{plan:planId,durationDays,boundTelegramId,maxDevices:Number(document.getElementById("ownerDevices").value||1),note:document.getElementById("ownerNote").value});const key=result.license?.license_key||"";status.textContent=key?`${t("created")}: ${key}${boundTelegramId?` · ID ${boundTelegramId}`:""}`:t("created");if(key){await copyText(key);toast(t("copied"));}await loadAdmin();}catch(e){status.textContent=humanError(e);}};
  }
