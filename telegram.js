(() => {
  ...
  function addInterfaceFixes(){
    ...
    style.textContent=`
      ...
      .precision-3-card{position:relative!important;border:1px solid rgba(52,211,153,.86)!important;background:linear-gradient(145deg,#061a1b,#10254a 52%,#251052)!important;box-shadow:0 16px 44px rgba(45,212,191,.27),inset 0 0 32px rgba(139,92,246,.18)!important;overflow:hidden!important}
      .precision-3-card .card-art{filter:saturate(1.3) hue-rotate(18deg) contrast(1.08) brightness(.62)!important}.precision-3-card .pro-badge{background:linear-gradient(135deg,#34d399,#38bdf8)!important;color:#041511!important}.precision-3-card .card-name{color:#e6fffb!important}.precision-3-card .card-studio{color:#99f6e4!important}
      .precision-3-card::after{content:"10X+ LIVE";position:absolute;right:10px;bottom:9px;z-index:5;padding:4px 8px;border-radius:999px;background:rgba(220,38,38,.24);border:1px solid rgba(251,113,133,.65);color:#fecdd3;font-size:10px;font-weight:900;letter-spacing:.1em}
  ...
  function makeCard({key,cls,badge,name,studio,file,aria,art="pro-luckyjet.jpg"}){
    ...
    card.innerHTML=`<img class="card-art" src="${art}" alt="${name}"><span class="pro-badge">${badge}</span><span class="card-copy"><span class="card-name">${name}</span><span class="card-studio">${studio}</span></span>`;
  ...
  function addProCards(){
    ...
    makeCard({key:"luckyjet-beta-v2",cls:"beta-v2-card",badge:"BETA V2",name:"LUCKY JET BIG X",studio:"TOPCOEFFICIENT • MAX DAY",file:"beta-v2/",aria:"Открыть Lucky Jet BETA V2 BIG X"});
    makeCard({key:"luckyjet-precision-3",cls:"precision-3-card",badge:"10X RADAR",name:"LUCKY JET 10X+",studio:"СИГНАЛ ДО 3 РАУНДОВ • LIVE",file:"precision-3/",aria:"Открыть Lucky Jet PRECISION 3 10X RADAR"});
